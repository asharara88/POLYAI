...
secondary_kpis:
  ...
  - Share of reservations from GCC investor segment vs. UK/India diaspora (target: 60/40)
audience:
  segment_a:
    name: GCC investor (yield + appreciation)
    pains:
      - Limited high-yield AED-denominated deployment options
      - Off-plan counter-party risk
      - Service-charge transparency
    current_alternatives:
      - Other tier-1 Dubai developers' off-plan launches
      - Existing-stock secondary purchase
      - Outside-region holdings (London, Singapore)
  segment_b:
    name: UK/India diaspora (NRI / UAE-affiliated UK)
    pains:
...
positioning:
  promise: |
    ...
    hospitality partner you already know.
  proof_points:
    ...
    - Escrow-protected payment plan compliant with RERA Article 11
...
offer:
  ...
  destination: meridianresidences.com/investor-pack (gated form, HubSpot)
channels:
  ...
  - channel: Meta (Instagram + Facebook)
    ...
    sub_audiences: [GCC HNI lookalikes, UK-Indian-diaspora interest stack]
  - channel: LinkedIn
    weight: 0.12
  ...
  - channel: Diaspora portals (Magicbricks IN, Rightmove UK)
    weight: 0.07
...
constraints:
  ...
  compliance_flags:
    - RERA            # routes through compliance for every public-facing artifact
    - off-plan-disclosure
    - financial-promotion
    - GDPR            # for UK diaspora outreach
measurement_plan:
  attribution_model: HubSpot revenue attribution (multi-touch, position-based weighting)
  test_design:
...
dependencies:
  ...
  - competitive-intel: snapshot of 3 competing Q3 launches in same district (assigned)
  - compliance: full pre-launch review of investor pack + landing page (mandatory)
  - localization: AR variants for primary creative; Hindi-context UK-diaspora variant (en-IN tone)
human_approval_required:
  - Final paid-channel launch (any spend > AED 25k/day per platform)
