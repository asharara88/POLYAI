...
- **Real-estate developer (off-plan launch)** → strategy + inventory-manager (baseline) → marketing-financial-manager (budget envelope) + marketing-procurement (vendor categories) → research + competitive-intel + voc → agency-liaison (engage external agencies) → brand-design + creative (review agency work) → compliance + localization (every artifact) → broker-enablement (channel materials, allocations, channel development) AND wealth-channel-enablement (HNI / UHNI parallel) AND vvip-channel-enablement (ruling-family / government / diplomatic) → events (per-event plans coordinated with all three channel agents) → email-lifecycle + social-media + seo (direct marketing, gated by inventory-manager) → review → analytics + forecasting (sustain) → integration-action envelopes for any CRM / ad / portal write
